...
WARN=False
INFO=False
PRINT_INTERMED_SORTS=False
FUZZ=False
PAIRS_OF_INTEREST=[('Sweden','Finland'),('Canada','Sweden'),('England','Australia'),('Scotland','Germany'),('USA','Japan')]
random.seed()
...
def doranking(results):
	teams = list(set([i[2].strip() for i in results]).union(set([i[0].strip() for i in results])))
	ratios = collections.defaultdict(dict)
	for i in results:
		names = (i[0].strip(),i[2].strip())
		#try:
		# 	tmp = ratios[names[0]][names[1]] 
		ratios[names[0]][names[1]] = logratio(i[1].strip(),i[3].strip())
		#except:
		#	ratios[names[0]][names[1]] = (logratio(i[1].strip(),i[3].strip()) + ratios[names[0]][names[1]])/2.0 
	ratios = [ (i[0].strip(),i[2].strip(),logratio(i[1].strip(),i[3].strip())) for i in results ] 
	...
	rankmatrix = { i: {j:[[None,1]]*SCFMAX for j in teams } for i in teams}
	...
	#			rankmatrix[i][j].append([None,0])
		...
		if INFO:
			...
			print "**************************************"
		if PRINT_INTERMED_SORTS and len([1 for i in teams for j in teams if rankmatrix[i][j][rank-1][0] is None]) == 0:
			print sorted(teams, lambda a,b : cmp(rankmatrix[a][b][rank-1][0],0) )
	convexity(teams, rankmatrix)
	
	if len(PAIRS_OF_INTEREST)>0:
		for pairitem in PAIRS_OF_INTEREST:
			print pairitem[0]+"v"+pairitem[1]
			print [(rank[1],math.exp(rank[0][0])) for rank in zip(rankmatrix[pairitem[0]][pairitem[1]],range(SCFMAX)) if rank[0][0] is not None]
	#print "USA v Japan"
	#print rankmatrix['USA']['Japan']
	#print
	#print "Scotland v Germany"
	#print rankmatrix['Scotland']['Germany']
	#Test topological sorts, return our best sort
	return sorted(teams, lambda x,y : cmp(rankmatrix[x][y][-1][0] if rankmatrix[x][y][-1][0] is not None else rankmatrix[x][y][-2][0],0) )
