...
#list of exhibition teams we want to include
exhibition_teams = ['88956','94513','78678','94900','94906','59540', '45049','88338','34322','20531','60981','51515','92900','43083' ]
...
#read from a processed teamvector psv
def import_teamvector(teamvector):
	t = open(teamvector,'rb')
	teams = {}
	
	for line in t:
		items = line.decode('utf_8').split('|')
		if items[3] != 'Travel Team' and items[3] != 'B Team':
			continue
		teams[items[0]] = (items[1], items[-2:], items[4]) 
	t.close()
	return teams
...
def import_teams(teamfile):
	...
	#		continue
		...
		if species != "Travel Team" and species != "B Team":
			#or if Latin American, include home teams, and also include select Exhibition teams, to work around FTS schema bollocks 
			if domain == u'' and species == "Home Team":
				pass
			elif items[0][1:] in exhibition_teams:
				pass
			else:
				continue
...
def select_bouts(bouts,teammap,startdate,enddate,trainingperiod):
	teamlist = teammap.keys()
	bouts_out = [list(),list()]
	...
	for (k,v) in bouts.iteritems(): 
		...
		for vv in v: 
			...
			if selector==0: #only add if this is a non-training bout [obvs, we want the teams to be the teams we can rank!]
				names.add(vv[0])
				names.add(vv[1])
				#boutgraph increment, with weighting for confidences
				boutgraph.add_edge(vv[0],vv[1], kfactor=(enddate-k)/datenormalise) #similarly, we can't reasonably include them in the boutgraph if not in training period
