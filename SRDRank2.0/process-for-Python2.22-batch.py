...
#
# 	batch version generates all "intermediate" rankings and ratings along the way
...
for date_i in range(1,len(dates)):
	...
	#if (not end_time):
	#	continue
	print "Outputting values Run"
	
	dates_list = [1,2,3,4,5] #range(int((final_epoch-datelim)/30*24*60*60)+1)
	#print dates_list
	...
	for d_tmp in dates_list:
		...
		for sg in subgraphs:
		        (maximal_bouts, maximal_boutgraph, maximal_names) = pfc.select_bouts(bouts_in, sg, datestart, d_item, 0)
		        #print sg
		...
		if (True):
			f=open('{0}.out'.format(date_string),'w')
			grp=0 #group id
